...
<template>
  ...
  <form @submit.prevent="fetchBooks2">
    <div><input placeholder="author..." type="text" v-model="q.inauthor" /> {{ q.inauthor }}</div>
    ...
    <div><input placeholder="subject..." type="text" v-model="q.subject" /> {{ q.subject }}</div>
    <input type="submit" />
    ...
    <ul v-if="state.booklist.length > 0 || state.booklist.totalItems > 0">
      <li v-for="(book, index) in state.booklist.items" :key="index">
        <div>
          ...
          <a v-if="book.volumeInfo.infoLink" :href="book.volumeInfo.infoLink" target="_blank"
            >&gt;</a
          >
        </div>
      </li>
